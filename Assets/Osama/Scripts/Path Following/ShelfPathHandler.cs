...
public class ShelfPathHandler : MonoBehaviour
{
    ...
    {
        return shelfPathTransforms[index].GetComponent<NodeRank>();
    }
    ...
    private void Awake()
    {
        ...
        shelvesOverPath = GetComponentsInChildren<ShelfObjectAlignerOverPath>();
    ...
    private void moveAccordingToScrollSpeed()
    {
        ...
        foreach (var scrollable in scrollables)
        {
            ...
            if (currentScrollSpeed > 0)
                nextTransformIndex = (scrollable.getObjectIndex() + 1) % shelfPathTransforms.Length;

            if (currentScrollSpeed < 0)
                nextTransformIndex = (scrollable.getObjectIndex() == 0) ? shelfPathTransforms.Length - 1 : scrollable.getObjectIndex() - 1;
